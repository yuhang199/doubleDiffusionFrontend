...
// Built per request: the Resend constructor throws on a missing key, which
// would fail the whole build rather than just this route.
function getResend() {
...
export async function POST(req) {
  try {
    ...
    if (!resend) {
      console.error("Contact form — RESEND_API_KEY is not set. Lead received but not emailed:", {
    ...
    if (error) {
      console.error("Contact form — Resend rejected the send:", {
        name: error.name,
